...
class Settings(BaseSettings):
    ...
    TEXIFY_BATCH_SIZE: int = 1 if TORCH_DEVICE_MODEL == "cpu" else 6 # Batch size for texify, lower on cpu due to float32
    TEXIFY_MODEL_NAME: str = "vikp/texify"

    # Layout model
    BAD_SPAN_TYPES: List[str] = ["Caption", "Footnote", "Page-footer", "Page-header", "Picture"]
    LAYOUT_MODEL_MAX: int = 128
    LAYOUT_CHUNK_OVERLAP: int = 32
    LAYOUT_DPI: int = 16
    LAYOUT_MODEL_NAME: str = "vikp/layout_segmenter"
    LAYOUT_BATCH_SIZE: int = 16 # Max 512 tokens means high batch size
